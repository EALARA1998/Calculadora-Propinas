...
export default function OrderTotals( {order, tip, dispatch} : OrderTotalsProps ) {
  ...
  return (
    <>
      ...
      <button className="w-full bg-black p-3 uppercase text-white font-bold mt-18 disabled:opacity-10" disabled={totalAmount === 0} onClick={()=>{dispatch({type: "place-order"})}}>Guardar Orden</button>
    </>
  )
}